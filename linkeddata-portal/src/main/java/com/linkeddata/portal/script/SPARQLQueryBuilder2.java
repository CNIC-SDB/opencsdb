 ...
 * @date : 2023/3/17 9:56
 */
public class SPARQLQueryBuilder2 {

    @Test
    public void buildQueriesTest() {
        String object1 = "https://www.plantplus.cn/plantsw/resource/Taxon_Alhagi_sparsifolia";
        String object2 = "http://chemdb.semweb.csdb.cn/resource/Compound_520-26-3";
        // 最大路径长度
        int maxDistance = 4;
        // sparql语句查询结果数量
        int limit = 10;
        List list = buildQueries(object1, object2, maxDistance, limit, null, null, 0);
        System.out.println(list);
   /*     for(int i=1;i<=5 ;i++){
            List<String> list1 = buildQueries(object1, object2, i, limit, null, null, 0);
            List<String> list2 = buildQueries(object2, object1, i, limit, null, null, 0);
            Set set = new HashSet();
            for (String sparql:list1 ) {
                set.add(sparql);
            }
            for (String sparql:list2 ) {
                set.add(sparql);
            }
            System.out.println("i="+i+" 路径条数"+set.size());
        }*/
    }
     ...
     * @param object1           资源1
     * @param object2           资源2
     * @param maxDistance       最大路径长度
     * @param limit             每条sparql语句查询结果限制
     * @param ignoredObjects    忽略的节点列表
     * @param ignoredProperties 忽略的属性列表
     * @param avoidCycles       是否避免环
     * @return
     */
    public List buildQueries(String object1, String object2, int maxDistance, int limit, List ignoredObjects, List ignoredProperties, int avoidCycles) {
       /*  List queries = new ArrayList<>();
       Map queries2 = getQueries(object1, object2, maxDistance, limit);
        queries2.forEach((key, value) -> {
            queries.add(value.toString());
        });*/
//        System.out.println(queries);
//        return queries;
        return getQueries(object1, object2, maxDistance, limit, ignoredObjects, ignoredProperties, avoidCycles);
    }
     ...
     */
    public List<String> getQueries(String object1, String object2, int maxDistance, int limit, List ignoredObjects, List ignoredProperties, int avoidCycles) {
...
//            queries.put(distance, direct(object1, object2, distance));
            list.add(direct(object1, object2, distance, options));
            ...
            for (int a = 1; a <= distance; a++) {
                ...
                for (int b = 1; b <= distance; b++) {
                    ...
                    if ((a + b) == distance) {
                        list.add(connectedViaAMiddleObject(object1, object2, a, b, true, options));
                        list.add(connectedViaAMiddleObject(object1, object2, a, b, false, options));
                    }
                }
     ...
     * @return
     */
    public String direct(String object1, String object2, int distance, Map options) {
        ...
        if (distance == 1) {
            ...
            vars.put("pred", predList);
            return completeQuery(query.toString(), options, vars);
        }
    }
     ...
     */
    public String connectedViaAMiddleObject(String first, String second, int dist1, int dist2, Boolean toObject, Map options) {
        Map vars = new HashMap();
        ...
        /* 将生成的
         ...
         拼接到 select * where {}
         */
        return completeQuery(coreQuery.toString(), options, vars);
    }
     ...
     * @return
     */
    private String completeQuery(String coreQuery, Map options, Map vars) {
